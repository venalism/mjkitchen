...
exports.placeOrder = async (req, res) => {
  ...
  try {
    // ✨ Your SQL script links order.user_id to auth.users(id), which is correct
    // The client will send profile.id as user_id, which is correct
  ...
  try {
    ...
    const { rows } = await pool.query(
      `SELECT o.*, p.name AS user_name 
         FROM "order" o 
         JOIN "profile" p ON p.id = o.user_id
        ORDER BY o.order_date DESC`
    );
    res.json(rows);
  } catch (e) {
