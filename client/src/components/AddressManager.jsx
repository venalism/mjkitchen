...
export default function AddressManager() {
  ...
  const loadAddresses = async () => {
    if (profile?.id) {
      try {
        const { data } = await api.get(`/users/${profile.id}/addresses`);
        setAddresses(data);
      } catch (error) {
  ...
  const handleModalSave = async (data) => {
    try {
      if (editingAddress) {
        // Update
        await api.put(`/users/${profile.id}/addresses/${editingAddress.address_id}`, data);
      } else {
        // Create
        await api.post(`/users/${profile.id}/addresses`, data);
      }
      handleModalClose();
  ...
  const handleDelete = async (addressId) => {
    if (window.confirm('Are you sure you want to delete this address?')) {
      try {
        await api.delete(`/users/${profile.id}/addresses/${addressId}`);
